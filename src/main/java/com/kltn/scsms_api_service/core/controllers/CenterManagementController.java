...
public class CenterManagementController {
    ...
    @GetMapping("/get-all")
    // Removed @RequireRole to allow Guest access (handled by SecurityConfiguration)
    @SwaggerOperation(summary = "Get all centers with filtering and pagination")
    @Operation(summary = "Get all centers with filtering and pagination", description = "Retrieve a paginated list of centers with optional filtering")
    public ResponseEntity<ApiResponse<PaginatedResponse<CenterInfoDto>>> getAllCenters(
            @Parameter(description = "Filter parameters for centers") @Valid CenterFilterParam centerFilterParam) {
    ...
    @Operation(summary = "Get center with all its branches", description = "Retrieve a specific center with all its associated branches")
    ...
    @Operation(summary = "Create new center", description = "Create a new car care center")
    public ResponseEntity<ApiResponse<CenterInfoDto>> createCenter(
            @Parameter(description = "Center creation request") @Valid @RequestBody CreateCenterRequest createCenterRequest) {
    ...
    @Operation(summary = "Update center", description = "Update an existing center")
    public ResponseEntity<ApiResponse<CenterInfoDto>> updateCenter(
            @Parameter(description = "Center ID") @PathVariable UUID centerId,
            @Parameter(description = "Center update request") @Valid @RequestBody UpdateCenterRequest updateCenterRequest) {
    ...
    @Operation(summary = "Delete center", description = "Delete a center (soft delete)")
