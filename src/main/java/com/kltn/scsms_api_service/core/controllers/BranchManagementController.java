...
public class BranchManagementController {
    ...
    @Operation(summary = "Get all branches with filtering and pagination", description = "Retrieve a paginated list of branches with optional filtering")
    public ResponseEntity<ApiResponse<PaginatedResponse<BranchInfoDto>>> getAllBranches(
            @Parameter(description = "Filter parameters for branches") @Valid BranchFilterParam branchFilterParam) {
    ...
    @Operation(summary = "Get branches by center ID", description = "Retrieve all branches belonging to a specific center")
    ...
    @Operation(summary = "Create new branch", description = "Create a new branch for a car care center")
    public ResponseEntity<ApiResponse<BranchInfoDto>> createBranch(
            @Parameter(description = "Branch creation request") @Valid @RequestBody CreateBranchRequest createBranchRequest) {
    ...
    public ResponseEntity<ApiResponse<BranchInfoDto>> updateBranch(
            @Parameter(description = "Branch ID") @PathVariable UUID branchId,
            @Parameter(description = "Branch update request") @Valid @RequestBody UpdateBranchRequest updateBranchRequest) {
    ...
    public ResponseEntity<ApiResponse<BranchInfoDto>> updateBranchStatus(
            @Parameter(description = "Branch ID") @PathVariable UUID branchId,
            @Parameter(description = "Branch status update request") @Valid @RequestBody UpdateBranchStatusRequest updateBranchStatusRequest) {

        log.info("Updating branch status for ID: {}", branchId);

        BranchInfoDto updatedBranch = branchManagementService.updateBranchActiveStatus(branchId,
