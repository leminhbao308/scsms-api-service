...
public interface SalesOrderRepository extends JpaRepository<SalesOrder, UUID>, JpaSpecificationExecutor<SalesOrder> {
     ...
     */
    @Query("SELECT so.id FROM SalesOrder so " +
            "WHERE so.customer.userId = :customerId " +
    ...
    @Query("SELECT DISTINCT so FROM SalesOrder so " +
            ...
            "LEFT JOIN FETCH l.product p " +
            "LEFT JOIN FETCH so.customer c " +
            "LEFT JOIN FETCH so.branch b " +
            "WHERE so.status = :status " +
            "AND so.createdDate >= :fromDate " +
            "AND so.createdDate <= :toDate " +
            "AND (:branchId IS NULL OR so.branch.branchId = :branchId) " +
            "ORDER BY so.createdDate ASC")
    List<SalesOrder> findSalesOrdersForReport(
            @Param("fromDate") LocalDateTime fromDate,
            @Param("toDate") LocalDateTime toDate,
            @Param("branchId") UUID branchId,
            @Param("status") SalesStatus status);

    List<SalesOrder> findByCreatedDateBetween(LocalDateTime createdDateAfter, LocalDateTime createdDateBefore);
}
