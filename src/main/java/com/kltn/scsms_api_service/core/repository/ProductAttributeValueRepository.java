...
public interface ProductAttributeValueRepository extends JpaRepository<ProductAttributeValue, ProductAttributeValueId>,
       ...
       /**
        * Batch fetch product attribute values for multiple products
        * Prevents N+1 queries when loading product attributes for sales orders
        * Fetches all attributes and their definitions in one query
        */
       @Query("SELECT pav FROM ProductAttributeValue pav " +
                     "JOIN FETCH pav.productAttribute pa " +
                     "WHERE pav.product.productId IN :productIds AND pav.isDeleted = false")
       List<ProductAttributeValue> findByProductIdInAndIsDeletedFalse(@Param("productIds") List<UUID> productIds);
}
